...
// Doit être identique à BASE_FOLDER dans src/lib/cloudinary.js
const BASE_FOLDER = 'elvis-portfolio/images';
const imagesDir = path.join(__dirname, '..', 'public', 'assets', 'images');

// Seules ces extensions sont uploadées ; le reste (.DS_Store, .json, etc.)
// est ignoré au lieu de planter l'upload dessus.
...
async function uploadAll() {
  ...
  const files = allFiles.filter(f => MEDIA_EXTENSIONS.has(path.extname(f).toLowerCase()));
  const skipped = allFiles.length - files.length;
  if (skipped > 0) {
    console.log(`ℹ️  ${skipped} fichier(s) ignoré(s) (extension non-média, ex: .DS_Store).\n`);
  }

  console.log(`🚀 Début de l'upload de ${files.length} fichiers vers Cloudinary (${cloudName}), dossier "${BASE_FOLDER}"...\n`);
